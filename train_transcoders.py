...
def main():
    ...
    # ensure that they are the new parameters and not the checkpoint
    hidden_matrix_params = [p for n, p in model.blocks.named_parameters() if p.ndim >= 2 and "embed" not in n and p.requires_grad]
    ...
    head_params = [model.lm_head.weight] if model.lm_head.weight.requires_grad else []
